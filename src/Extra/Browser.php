...
class Browser extends \React\Http\Browser
{
    private $transaction;
    private $baseUrl;
    ...
    public function __construct(LoopInterface $loop, $proxy = null)
    {
        $this->proxy = $proxy;

        if($proxy) {
            $proxy = new \Galdino\Proxy\Extra\Proxy($proxy, new Connector($loop, [
                'dns' => '1.1.1.1',
                'tls' => array(
                    'verify_peer' => false,
        ...
        $this->transaction = new \Galdino\Proxy\Extra\HttpProtocol\Transaction(
            \Galdino\Proxy\Extra\HttpProtocol\Sender::createFromLoop($loop, $connector),
            $loop
        );
    ...
    private function requestMayBeStreaming($method, $url, array $headers = array(), $body = '')
    {
        ...
        return $this->transaction->send(
            new \Galdino\Proxy\Extra\HttpProtocol\Request($method, $url, $headers, $body, $this->protocolVersion, $this->proxy)
        );
    }
}